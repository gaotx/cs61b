...
public class SplayTree implements Dictionary {
  ...
  private void splayNode(BinaryTreeNode node) {
    ...
    if (node.parent == root) {
      zig(node);
    } else if (g.leftChild != null && g.leftChild.leftChild != null && node == g.leftChild.leftChild) {
      zigZig(node);
      splayNode(node);
    } else if (g.rightChild != null && g.rightChild.rightChild != null && node == g.rightChild.rightChild) {
      zigZig(node);
      splayNode(node);
    } else if (g.leftChild != null && g.leftChild.rightChild != null && node == g.leftChild.rightChild) {
      zigZag(node);
      splayNode(node);
    } else if (g.rightChild != null && g.rightChild.leftChild != null && node == g.rightChild.leftChild) {
      zigZag(node);
      splayNode(node);
    }
