...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int size = 0;
    for (i = 0; i < horiz; i++) {//hWalls
      for (j = 0; j < vert-1;j++) {
        walls[size] = new Wall(i, j, "horiz");
        size++;
      }
    }
    for (i = 0; i < horiz-1; i++) {//vWalls
      for (j = 0; j < vert; j++) {
        walls[size] = new Wall(i, j, "vert");
        size++;
      }
    }
    // Scramble the walls
    int w, select;
    for (w = walls.length; w > 1; w--) {
      select = randInt(w);// select = 0, 1, ..., w-1
      swap(walls, select, w-1);
    }
    // Visit walls
    Wall wall;
    int root1, root2;
    for (int k = 0; k < walls.length; k++) {
      wall = walls[k];
      if (wall.getType().equals("horiz")) {
        root1 = horiz * wall.getY() + wall.getX();
        root2 = horiz * (wall.getY() + 1) + wall.getX();
      } else {
        root1 = horiz * wall.getY() + wall.getX();
        root2 = horiz * wall.getY() + wall.getX() + 1;
      }
      // check if they are connected
      root1 = cells.find(root1);
      root2 = cells.find(root2);
      if (root1 != root2) {// unconnected yet
        System.out.println("connecting " + root1 + " and " + root2);
        try {
        ...
        if (wall.getType().equals("horiz")) {
          hWalls[wall.getX()][wall.getY()] = false;
        } else {
          vWalls[wall.getX()][wall.getY()] = false;
        }
      }
    }
  }

  private void swap(Wall[] lst, int p1, int p2) {
    Wall temp = lst[p1];
    lst[p1] = lst[p2];
    lst[p2] = temp;
  }
