...
public class Sorts {
   ...
   **/
  private static final int DIGITSIZE = 4;
  private static final int RADIX = 16; // 2^DIGITSIZE
  private static final int BITMASK = RADIX - 1; // binary 1111
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    ...
    for (int i = 0; i < keys.length; i++) {
      counts[((keys[i] >> (whichDigit * DIGITSIZE)) & BITMASK)]++;
    }
    int total = 0;
    ...
    for (int i = 0; i < keys.length; i++) {
      y[counts[((keys[i] >> (whichDigit * DIGITSIZE)) & BITMASK)]] = keys[i];
      counts[((keys[i] >> (whichDigit * DIGITSIZE)) & BITMASK)]++;
    }
    return y;
  }
