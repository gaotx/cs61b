...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    for (int i = 0; i < ints.length && ints[i] != -1; i++) {
      int j = i + 1;
      while(j < ints.length && ints[i] == ints[j]) j++;
      if (j - 1 > i) {
        int m = i + 1, n = j, times = ints.length - j;
        for (; times > 0; times--) ints[m++] = ints[n++];// replace
        for (; m < ints.length; m++) ints[m] = -1;// set unused
      }
    }
  }
  ...
  public static void main(String[] args) {
    ...
    int[] myTest1 = {};
    System.out.println("smooshing " + stringInts(myTest1) + ":");
    smoosh(myTest1);
    result = stringInts(myTest1);
    System.out.println(result);
    TestHelper.verify(result.equals("[  ]"),
                      "BAD SMOOSH!!!  No cookie.");

    int[] myTest2 = {1};
    System.out.println("smooshing " + stringInts(myTest2) + ":");
    smoosh(myTest2);
    result = stringInts(myTest2);
    System.out.println(result);
