...
def _check_nums(*vals):
    ...
    for i, v in enumerate(vals):
        if type(v) is not complex and not scheme_numberp(v):
...
@primitive("round")
def scheme_round(x):
	"""Adds a primitive for the round function."""
	return round(x)

@primitive("tan")
def scheme_tan(z):
	"""Provides complex number support for tan(x)."""
	return cmath.tan(z)

@primitive("complex-sub")
def complex_sub(z1, z2):
	"""Provides complex number support for subtraction."""
	return z1 - z2

@primitive("complex-div")
def complex_div(z1, z2):
	"""Provides complex number support for division."""
	return z1/z2

@primitive("complex-abs")
def complex_abs(z):
